  ...
  function buildRecord(row){
    ...
    if(type === 'req'){
      ...
      const priority = row.dataset.priority || textFrom(row.children[3]) || 'Normal';
      return {type, key:ref, title:`Requisition · ${ref}`, ref, item:textFrom(row.children[1]), qty:Number(textFrom(row.children[2])) || 0, priority, delivery:textFrom(row.children[3]), dept:textFrom(row.children[4]), requester:textFrom(row.children[5]), status:textFrom(row.children[6]), date:textFrom(row.children[7]), time:row.dataset.time || '10:30 AM', uom:row.dataset.uom || 'pcs', notes:row.dataset.notes || `Requested for ${textFrom(row.children[4])} operations.`, po:textFrom(row.dataset.po || ''), hasPO: row.dataset.hasPo === '1'};
    }
    if(type === 'invoice'){
  ...
  function renderViewRecord(row){
    ...
    if(record.type === 'po'){
      ...
      const poBtn = record.hasPO ? null : {label:'Create Purchase Order', className:'btn-primary', onClick:()=>{ convertReqToPO(record.ref, record.item, record.qty, record.priority); closeViewModal(); }};
